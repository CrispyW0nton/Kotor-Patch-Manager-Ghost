...
animation name `victory`. For local testing, install a supermodel MDL/MDX pair
containing that animation in Override. This is only for proving the hook
contract; release demos should use explicit resolver keys and package their asset
files through KPM's `additional/` flow once that installer path exists.
